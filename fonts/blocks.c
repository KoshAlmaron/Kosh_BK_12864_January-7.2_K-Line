...
void draw_no_signal() {
	...
	char Test[] = "АБВГДЕЖЗИЙ";

	oled_set_font(u8g2_font_cyrillic_5x8_tr);
	oled_print_string(0, 1, "+01234-5678.9", 13);
	oled_print_string(0, 20, "АБВГДЕЖЗИЙКЛМНОПРСТУ", 20);
	oled_print_string(0, 40, "ФХЦЧШЩЪЫЬЭЮЯ", 12);
...
void draw_O2_sensor_f(uint8_t x, uint8_t y) {
	...
	uint8_t LambdaType;
	uint16_t AFR = (147 * (uart_get_uint8(12) + 128)) >> 8; // Значение x10.
	if (AFR > 0) {
		...
		LambdaType = 0;
		AFR = (125 * uart_get_uint8(23)) >> 8; // Значение x100.
	...
	if (!ErrorsCount) {oled_print_string_f(29, 28, NoErrors, 9);}
		...
		for (uint8_t i = StartRow; i < ErrorsCount; i++) {
			if (Row < CE_ROWS_ON_SCREEN) {
				...
				Row++;
				if (Col == 0 && Row == CE_ROWS_ON_SCREEN) {	// Вторая колонка.
					Col = 1;
					Row = 0;
				}
			}
		...
		if (CE_ROWS_ON_SCREEN + StartRow + Col * CE_ROWS_ON_SCREEN < ErrorsCount) {
			...
			if (ButtonState[1] == 201) {StartRow += CE_ROWS_ON_SCREEN * 2;}	// Смещение вниз.
		}
